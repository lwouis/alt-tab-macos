...
class AppCenterCrash: NSObject, CrashesDelegate {
    ...
    // we need to call this again if the setting changes so make it a function
    func setDefaultNetworkRequestsPolicy(override: Bool = false) {
        // this will default to false thus not sending anything until opted-in
        // override is to override temporarily on a case-by-case basis
        AppCenter.networkRequestsAllowed = override || Preferences.crashPolicy == .always
    }
    ...
    func initAppCenter() {
        ...
        self.setDefaultNetworkRequestsPolicy()
    ...
    func confirmationHandler(errorReports: [ErrorReport]) -> Bool {
        ...
        if Preferences.crashPolicy == .ask {
            ...
            let shouldSend = userChoice == .alertFirstButtonReturn
            
            self.setDefaultNetworkRequestsPolicy(override: shouldSend)
            
            BackgroundWork.crashReportsQueue.async { Crashes.notify(with: shouldSend ? .send : .dontSend) }
        } else {
