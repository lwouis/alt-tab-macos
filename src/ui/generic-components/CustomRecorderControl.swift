...
class CustomRecorderControl: RecorderControl, RecorderControlDelegate {
    ...
    func alertIfSameShortcutAlreadyAssigned(_ shortcut: Shortcut) {
        if let shortcutAlreadyAssigned = (ControlsTab.shortcuts.values.first {
            ...
            if shortcutAlreadyAssigned.id.starts(with: "cycleSelectionLeftVim") {
                alert.informativeText = String(format: NSLocalizedString("Shortcut already assigned in window selection with Vim keys.", comment: ""))
                alert.runModal()
                return
            }
