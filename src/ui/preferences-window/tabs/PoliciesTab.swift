...
class PoliciesTab {
    ...
    static func initTab() -> NSView {
        let updateLabel = LabelAndControl.makeLabel(
            NSLocalizedString("Updates policy:", comment: ""))
        PoliciesTab.updateButtons = LabelAndControl.makeRadioButtons(
            UpdatePolicyPreference.allCases, "updatePolicy",
            extraAction: { _ in
        ...
        let checkForUpdates = NSButton(
            title: NSLocalizedString("Check for updates now…", comment: ""), target: nil,
            action: #selector(PoliciesTab.checkForUpdatesNow))

        let crashLabel = LabelAndControl.makeLabel(
            NSLocalizedString("Crash reports policy:", comment: ""))
        PoliciesTab.crashButtons = LabelAndControl.makeRadioButtons(
            CrashPolicyPreference.allCases, "crashPolicy")
