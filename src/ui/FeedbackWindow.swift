...
class FeedbackWindow: NSWindow {
    ...
    private func setupView() {
        ...
        let appText = StackView(
            [
                BoldLabel(
                    NSLocalizedString("Share improvement ideas, or report bugs", comment: "")),
                HyperlinkLabel(
        ...
        sendButton = NSButton(
            title: NSLocalizedString("Send", comment: ""), target: nil,
            action: #selector(sendCallback))
        sendButton.keyEquivalent = "\r"
        sendButton.isEnabled = false
        let buttons = StackView([
            NSButton(
                title: NSLocalizedString("Cancel", comment: ""), target: nil,
                action: #selector(cancel)),
            sendButton,
        ])
        buttons.spacing = GridView.interPadding
        body = TextArea(
            80, 12, NSLocalizedString("I think the app could be improved with…", comment: ""),
            { () -> Void in
                self.sendButton.isEnabled = !self.body.stringValue.isEmpty
            })
        email = TextArea(
            80, 1, NSLocalizedString("Optional: email (if you want a reply)", comment: ""))
        debugProfile = NSButton(
            checkboxWithTitle: NSLocalizedString(
                "Send debug profile (CPU, memory, etc)", comment: ""), target: nil, action: nil)
        debugProfile.state = .on
        let warning = BoldLabel(
            NSLocalizedString(
                "All data from this form will be made public, as a ticket on github.com",
                comment: ""))
        let view = GridView([
            [header],
            [body],
    ...
    func warnAboutNoEmail() -> Bool {
        ...
        alert.messageText = NSLocalizedString(
            "Are you sure you don’t want a response?", comment: "")
        alert.informativeText = NSLocalizedString(
            "You didn’t write your email, thus can’t receive any response.", comment: "")
    ...
    private func prepareRequest() -> URLRequest {
        var request = URLRequest(
            url: URL(string: "https://api.github.com/repos/lwouis/alt-tab-macos/issues")!)
