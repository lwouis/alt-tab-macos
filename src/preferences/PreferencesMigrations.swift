class PreferencesMigrations {
    ...
    static func migratePreferences() {
        ...
        let existingVersion = UserDefaults.standard.string(forKey: preferencesKey)
    ...
    private static func migrateGroupingToPerShortcut() {
        if let old = UserDefaults.standard.string(forKey: "showAppsOrWindows") {
            ...
            UserDefaults.standard.removeObject(forKey: "showAppsOrWindows")
        }
        let oldGlobal = UserDefaults.standard.string(forKey: "showTabsAsWindows")
        ...
        for i in 0...Preferences.maxShortcutCount {
            ...
            if let existing = UserDefaults.standard.string(forKey: key) {
                if existing == "true" || existing == "false" {
                    UserDefaults.standard.set(existing == "true" ? "1" : "0", forKey: key)
                }
            } else if let convertedGlobal {
                UserDefaults.standard.set(convertedGlobal, forKey: key)
            }
        }
        if oldGlobal != nil {
            UserDefaults.standard.removeObject(forKey: "showTabsAsWindows")
        }
    }
    ...
    private static func migrateLanguagePreferenceIndex() {
        ...
        guard let stored = UserDefaults.standard.string(forKey: "language"), let oldIndex = Int(stored) else { return }
        let newIndex = oldToNew[oldIndex] ?? 0
        UserDefaults.standard.set(String(newIndex), forKey: "language")
    }
    ...
    private static func migrateExceptionsTitleArray() {
        ...
        guard let raw = UserDefaults.standard.string(forKey: "exceptions"),
        ...
        UserDefaults.standard.set(Preferences.jsonEncode(migrated), forKey: "exceptions")
    }
