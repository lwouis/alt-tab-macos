...
flowchart TD
    ...
    Fork -->|"Hard-gated feature triggered<br/>(search, lockSearch, shortcut ≥ 2)"| FP["Free pass<br/>Execute action once"]
...
| Lock search shortcut | Hard-gated | Blocked; free-pass → [C], then [E] | Yes |

### `isProAvailable` vs `isProLocked`

- `isProAvailable` — true for `.trial` and `.pro`. Used by hard-gate entry points (search, lockSearch, `+` shortcut beyond 1) to decide whether to execute directly or consult the free-pass ladder.
