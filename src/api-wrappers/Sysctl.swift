...
public struct Sysctl {
    ...
    static func run<T>(_ name: String, _ type: T.Type) -> T? {
        return run(
            name, { $0.baseAddress?.withMemoryRebound(to: T.self, capacity: 1) { $0.pointee } })
    }
    ...
    private static func data(_ keys: [Int32]) -> [Int8]? {
        return keys.withUnsafeBufferPointer { keysPointer in
            ...
            let preFlightResult = Darwin.sysctl(
                UnsafeMutablePointer<Int32>(mutating: keysPointer.baseAddress), UInt32(keys.count),
                nil, &requiredSize, nil, 0)
            ...
            let result = data.withUnsafeBufferPointer { dataBuffer -> Int32 in
                return Darwin.sysctl(
                    ...
                    UInt32(keys.count), UnsafeMutableRawPointer(mutating: dataBuffer.baseAddress),
                    &requiredSize, nil, 0)
            }
            if result != 0 {
                return nil
    ...
    private static func keys(_ name: String) -> [Int32]? {
        ...
        _ = keysBuffer.withUnsafeMutableBufferPointer {
            (lbp: inout UnsafeMutableBufferPointer<Int32>) in
